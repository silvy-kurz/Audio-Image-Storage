...
afi_result_t map_frames_to_pixels(afi_frames_t *frames, afi_pixels_t **pixels_out) {
  ...
  switch (frames->type) {
    ...
    case STEREO_16 : {
      ...
      for (pixel_index = 0; pixel_index < full_pixels_count; pixel_index++) {
        ...
        pixel_buffer[pixel_index].r = (u32)(frame_buffer[frame_index].left_sample   | (frame_buffer[frame_index].right_sample << 16));
        pixel_buffer[pixel_index].g = (u32)(frame_buffer[frame_index + 1].left_sample   | (frame_buffer[frame_index + 1].right_sample << 16));
        pixel_buffer[pixel_index].b = (u32)(frame_buffer[frame_index + 2].left_sample   | (frame_buffer[frame_index + 2].right_sample << 16));
        pixel_buffer[pixel_index].a = (u32)(frame_buffer[frame_index + 3].left_sample   | (frame_buffer[frame_index + 3].right_sample << 16));
      }
      ...
      for (int p = 0; p < padding_pixels; p++) {
          ...
          for (int s = 0; s < left_over_frames; s++) {
              channels[s] = (u32)(frame_buffer[frame_index + s].left_sample | (frame_buffer[frame_index + s].right_sample << 16));
          }
