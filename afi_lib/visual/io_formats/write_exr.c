...
void
write_rgba_uint32_exr (const char *filename, int width, int height, const afi_pixel_u32_t *rgba_pixels)
{
  ...
  for (int y = 0; y < nchunks; y++)
    {
      ...
      if (rv != EXR_ERR_SUCCESS)
        printf (" scanline chunk info failed at chunk %d!\n", y);

      if (first)
        {
          rv = exr_encoding_initialize (myfile, 0, &cinfo, &encoder);
          if (rv != EXR_ERR_SUCCESS)
            printf ("huge error at chunk init %d!\n", y);
        }
      else
        {
          rv = exr_encoding_update (myfile, 0, &cinfo, &encoder);
          if (rv != EXR_ERR_SUCCESS)
            printf ("huge error at chunk update %d!\n", y);
        }
      ...
      if (first)
        {
          ...
          if (rv != EXR_ERR_SUCCESS)
            printf ("huge error at chunk routine %d!\n", y);
        }
      rv = exr_encoding_run (myfile, 0, &encoder);
      if (rv != EXR_ERR_SUCCESS)
        printf ("encoding run failed at chunk %d with error %d\n", y, rv);
